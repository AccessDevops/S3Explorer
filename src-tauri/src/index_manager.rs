...
impl IndexManager {
    ...
    pub async fn initial_index_bucket(
        &self,
        adapter: &S3Adapter,
        bucket_name: &str,
        ...
        if is_complete {
            ...
            let unique_prefixes = self.db.get_unique_parent_prefixes(bucket_name)?;
            let now = chrono::Utc::now().timestamp_millis();

            for prefix in unique_prefixes {
                if prefix.is_empty() {
                    continue; // La racine est gérée séparément
                }

                // Calculer les stats pour ce prefix
                let (count, size) = self.db.calculate_prefix_stats(bucket_name, &prefix)?;
...
lazy_static::lazy_static! {
    /// Cache global des gestionnaires d'indexation
    static ref INDEX_MANAGERS: RwLock<HashMap<String, Arc<IndexManager>>> =
...
pub fn get_index_manager(profile_id: &str) -> Result<Arc<IndexManager>, AppError> {
    ...
    let manager = Arc::new(IndexManager::new(profile_id)?);

    // Stocker dans le cache
    {
        let mut cache = INDEX_MANAGERS
            .write()
            .map_err(|e| AppError::IndexError(e.to_string()))?;
...
#[allow(dead_code)]
pub fn close_index_manager(profile_id: &str) -> Result<(), AppError> {
    let mut cache = INDEX_MANAGERS
        .write()
        .map_err(|e| AppError::IndexError(e.to_string()))?;
